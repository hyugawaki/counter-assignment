  ...
  <div class="grid grid-cols-1 gap-2">
    {#each counters as { component, count }, i}
      <div class="flex justify-center">
        ...
        <button on:click={() => deleteCounter(i)} class="ml-2"
          ><IconBackspaceFilled /></button
        >
      </div>
    {/each}
  </div>
  <button
    on:click={() => addCounter(0)}
    class="rounded my-4 px-2 w-1/4 bg-amber-100">Add Counter</button
  >
